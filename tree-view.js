(function () {
  ...
  async function loadAndRenderTree(familyId, canvas, statusNode, detailPanel, detailEmpty) {
    ...
    try {
      const graph = await window.TOLAuth.apiRequest(`/families/${familyId}/graph`, {
        method: 'GET'
      });

      renderStructuredTree(graph, canvas, detailPanel, detailEmpty);
      showStatus(statusNode, 'Visual family tree loaded successfully.', 'success');
    } catch (error) {
  ...
  function renderStructuredTree(graph, canvas, detailPanel, detailEmpty) {
    ...
    if (!members.length) {
      canvas.innerHTML = '<div class="tree-empty">No members found for this family.</div>';
      return;
    }
    ...
    const sortedRows = Array.from(generationGroups.keys()).sort((a, b) => a - b);
    ...
    rows.forEach((row, visualRowIndex) => {
      const y = topPadding + visualRowIndex * rowGap;
      ...
      const enrichedItems = row.items.map(item => {
        ...
        const widthValue = item.type === 'couple' ? (nodeWidth * 2 + coupleGap) : nodeWidth;
      ...
      let currentX = leftPadding + ((width - leftPadding - rightPadding) - rowWidth) / 2;
    ...
    const childLinks = relationships.filter(rel => rel.relationship_type === 'parent_child');
    const spouseLinks = relationships.filter(rel => rel.relationship_type === 'spouse');

    const spouseSet = new Set(
      spouseLinks.map(rel => buildPairKey(rel.source_member_id, rel.target_member_id))
    );
    ...
    const groupedChildIds = new Set(
      Array.from(groupedChildren.values()).map(link => `${link.parentA}::${link.childId}`)
        .concat(Array.from(groupedChildren.values()).map(link => `${link.parentB}::${link.childId}`))
    );
  ...
  function placeMemberNode(wrapper, member, x, y, width, height, detailPanel, detailEmpty, memberMap, relationships) {
    ...
    const name = member.display_name || `${member.first_name || ''} ${member.last_name || ''}`.trim();
  ...
  function renderDetailPanel(member, memberMap, relationships, detailPanel, detailEmpty) {
    ...
    const spouses = relationships
      .filter(rel =>
        ...
        (rel.source_member_id === member.id || rel.target_member_id === member.id)
      )
      .map(rel => rel.source_member_id === member.id ? rel.target_member_id : rel.source_member_id)
      .map(id => memberMap.get(id))
      .filter(Boolean);

    const parents = relationships
      .filter(rel => rel.relationship_type === 'parent_child' && rel.target_member_id === member.id)
      .map(rel => memberMap.get(rel.source_member_id))
      .filter(Boolean);

    const children = relationships
      .filter(rel => rel.relationship_type === 'parent_child' && rel.source_member_id === member.id)
  ...
  function getItemAnchorX(item, positions, relationships, pairMap) {
    ...
    memberIds.forEach(memberId => {
      const parentLinks = relationships.filter(
        rel => rel.relationship_type === 'parent_child' && rel.target_member_id === memberId
      );

      if (!parentLinks.length) return;

      const parentIds = [...new Set(parentLinks.map(rel => rel.source_member_id))];
      const parentCenters = [];
    ...
    if (anchorValues.length) {
      return anchorValues.reduce((sum, value) => sum + value, 0) / anchorValues.length;
    }
  ...
  function buildMemberRowMap(members, relationships) {
    ...
    members.forEach(member => {
      ...
      const baseRow = component.reduce((minRow, memberId) => {
        ...
        const currentGen = Number.isInteger(currentMember?.generation) ? currentMember.generation : 0;
    ...
    members.forEach(member => {
      if (!memberRowMap.has(member.id)) {
        memberRowMap.set(member.id, Number.isInteger(member.generation) ? member.generation : 0);
      }
    });
